...
const Project = ({ project }) => {
    return (<div className="group hover:-translate-y-1 overflow-hidden transition-all ease-in-out z-50 w-full px-8 py-8 rounded-xl relative" style={{ backgroundColor: project.theme.color }}>
        ...
        <h2 className="text-3xl font-medium z-50 uppercase flex items-center" style={{ color: project.theme.text }}>
            {project.title}
            {project.tags && project.tags.map((tag, index) => (<span key={index} className="text-xs ml-2 backdrop-blur-sm bg-white/20 p-1 px-2 rounded-lg">
                {tag}
            </span>))}
        </h2>
        ...
        {project.image && <div className="hidden md:flex -z-10 absolute top-0 bottom-0 group-hover:right-2 -right-[64px] transition-all ease-[cubic-bezier(0.175,0.885,0.32,1.275)] duration-[.8s] items-center">
